"""
SAMURAI + YOLO pipeline — SAMURAI video propagation for masks + YOLO keypoints.

Architecture:
  - SAMURAI (SAM2 video predictor) provides masks and identity via propagate_in_video
  - YOLO provides 7 keypoints per rat (detect_only, no tracking — SAMURAI owns identity)
...
Strategy:
  ...
  - YOLO keypoints applied per-frame during streaming

Usage:
    python -m src.pipelines.samurai.run --config configs/hpc_samurai.yaml
    python -m src.pipelines.samurai.run --config configs/hpc_samurai.yaml --interactive
"""
...
# ---------------------------------------------------------------------------
# YOLO keypoint-to-mask assignment
# ---------------------------------------------------------------------------

def assign_keypoints_to_masks(
    detections: List[Detection],
    masks: List[Optional[np.ndarray]],
    centroids: List[Optional[Tuple[float, float]]],
    anchor_keypoint: str = "mid_body",
) -> List[Optional[Detection]]:
    """Assign YOLO detections to SAMURAI masks using a single anchor keypoint.

    YOLO's keypoint grouping (which keypoints belong to which rat) is good —
    the problem is only knowing which YOLO detection is which rat. We resolve
    this by checking which mask contains the anchor keypoint (mid_body).

    The entire YOLO detection (all 7 keypoints including tail) is kept intact.
    Only identity assignment comes from the mask.
    """
    n_slots = len(masks)
    slot_dets: List[Optional[Detection]] = [None] * n_slots
    used_dets = set()

    # Pass 1: assign by anchor keypoint containment in mask
    for det in detections:
        if det.keypoints is None:
            continue

        # Find the anchor keypoint (mid_body)
        anchor = None
        ...
        if anchor is None or anchor.conf < 0.1:
            continue

        ix, iy = int(round(anchor.x)), int(round(anchor.y))

        for i, mask in enumerate(masks):
            if mask is None or slot_dets[i] is not None:
                continue
            h, w = mask.shape
    ...
    # Pass 2: nearest centroid fallback for unassigned detections
    for det in detections:
        if id(det) in used_dets or det.keypoints is None:
            continue
        ...
        for i in range(n_slots):
            if slot_dets[i] is not None or centroids[i] is None:
                continue
...
def _extract_chunk_frames(
    ...
    """Copy a range of frames into a chunk-specific directory with re-indexed names."""
...
def run_pipeline(
    ...
    tag = f"samurai_chunk{chunk_id}" if chunk_id is not None else "samurai"
    run_dir = setup_run_dir(config, tag=tag)
    setup_logging(run_dir)

    logger.info("Starting SAMURAI + YOLO pipeline")
    ...
    out_video_path = overlays_dir / f"samurai_{today}{ext}"
        ...
        with torch.inference_mode(), ctx:
            ...
            for local_frame_idx, obj_ids, mask_logits in tqdm(
                ...
                yolo_dets = detect_only(
                    yolo_model, frame_rgb, confidence=yolo_conf,
                    keypoint_names=kpt_names,
                )
                if yolo_dets:
                    yolo_detect_count += 1
                    slot_dets = assign_keypoints_to_masks(
                        yolo_dets, slot_masks, slot_centroids,
                    )
                ...
                status = f"F{global_frame_idx} | Animals: {active_count}/{max_animals} | SAMURAI+YOLO: {kpt_count} det"
    ...
    logger.info("Frames with YOLO detections: %d/%d (%.1f%%)",
                yolo_detect_count, frame_count,
                100 * yolo_detect_count / max(frame_count, 1))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="SAMURAI + YOLO pipeline: SAMURAI mask propagation + YOLO keypoints.",
    )
